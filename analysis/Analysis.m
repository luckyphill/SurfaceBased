classdef (Abstract) Analysis < matlab.mixin.SetGet
	...
	methods
		...
		function SetSaveDetails(obj)
			...
			faceBasedPath = getenv('FACEDIR');
			if isempty(faceBasedPath)
				error('FACEDIR environment variable not set');
			end
			if ~strcmp(faceBasedPath(end),'/')
				faceBasedPath(end+1) = '/';
			end

			

			obj.imageSaveLocation = [faceBasedPath, 'Images/', obj.analysisName, '/'];

			obj.dataSaveLocation = [faceBasedPath, 'AnalysisOutput/', obj.analysisName, '/'];
			...
			if obj.usingHPC
				obj.simulationFileLocation = [faceBasedPath, 'HPC/', obj.analysisName, '/'];
